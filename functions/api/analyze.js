...
function applyTierVisibility(data, plan, modo) {
  ...
  // Starter: veredicto + señales + 1 acción prioritaria
  const starter = {
    _plan:               "starter",
    ...
    // Top 3 señales: alertas más críticas
    alertas:             (data.alertas || []).slice(0, 3),
    // 1 acción prioritaria
    recomendaciones:     (data.recomendaciones || [])
                           .filter(r => r.prioridad === "Alta")
                           .slice(0, 1),
    // Fortalezas: máximo 2 para dar valor sin dar todo
    fortalezas:          (data.fortalezas || []).slice(0, 2),
    // El resto locked
    _locked: {
      atsDetalle:           true,

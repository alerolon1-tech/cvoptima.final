...
export async function onRequest(context) {
  ...
  try {
    ...
    // Los 5 módulos exclusivos de Pro (capital relacional, trayectoria,
    // posicionamiento, narrativa, ECS) se piden aparte, desde un endpoint
    // separado (/api/analisis-pro-extra) que el frontend llama después de
    // mostrar este resultado — así la persona ve su panel rápido, sin esperar
    // el segundo pedido antes de ver nada.
...
function buildPromptES(cvText, liText, modo, role, sector, seniority, plan, situacion = '', includeProExtra = true) {
  ...
  instrBlock += "CRÍTICO — NO CONTRADIGAS EL DOCUMENTO: antes de escribir una debilidad que diga que algo 'falta' o 'no está' (por ejemplo 'faltan logros cuantificados', 'no hay métricas'), releé el documento COMPLETO buscando esa información — no solo la sección o el puesto que estás describiendo en ese momento. Si el dato SÍ aparece en otra parte del CV (otro puesto, otra sección), la debilidad tiene que ser específica sobre DÓNDE falta ('esta experiencia puntual no incluye métricas, a diferencia de otras en tu CV'), nunca una afirmación general de ausencia que contradiga lo que el documento sí muestra en otro lado. Una debilidad que contradice el contenido real del CV es un error grave.\n\n";
  instrBlock += "ROL VS. INDUSTRIA — NO LOS CONFUNDAS: identificá el ROL o FUNCIÓN real de la persona (qué hace: investigador, analista, coordinador, consultor, etc.) por separado del SECTOR o INDUSTRIA donde lo aplicó (inmobiliario, minería, retail, tecnología). Si la persona tuvo el mismo rol o función en distintos sectores, ESO es el dato central de su perfil — nombralo explícitamente ('es investigador/a social, con experiencia aplicando ese rol en inmobiliario, minería, retail y tecnología'), no lo diluyas en una observación genérica de 'adaptabilidad a distintas industrias' que invisibiliza cuál es su especialización real. Esto es especialmente importante en perfiles poco habituales o que no siguen una carrera corporativa lineal — no fuerces la lectura hacia una interpretación genérica cuando el documento muestra una especialización clara y consistente.\n\n";
  ...
  instrBlock += "CÓMO ESCRIBIR resumenEjecutivo (el veredicto principal): es puramente interpretativo — NO incluyas ningún número en este campo, ni siquiera entre paréntesis. Nada de atsScore, scorePotencial, impactDensityScore ni ninguna otra cifra. Contá en palabras cómo se lee la trayectoria hoy, qué funciona y qué no, y hacia dónde podría moverse con los ajustes sugeridos — todo en prosa, sin una sola cifra. Ejemplo del tono a seguir (adaptalo a los datos reales de este documento, no lo copies literal): \"Hoy tu trayectoria está siendo leída con [síntesis de cómo se lee: claridad parcial / con fuerza / de forma fragmentada, etc.] — tu experiencia en [área] es [cualidad], pero [lo que limita el impacto, en palabras]. Con los ajustes que se muestran más abajo, esa lectura podría [mejorar de forma significativa / cambiar considerablemente]. Lo que más pesa en esa diferencia es [la razón concreta basada en el documento], lo que hoy ubica tu perfil en un nivel [alto/medio/bajo] de densidad de impacto.\"\n";
  instrBlock += "OBLIGATORIO: antes de responder, revisá el texto de resumenEjecutivo y confirmá que no contenga ningún dígito (0-9) en ningún lado. Si encontrás uno, sacalo y reescribí esa parte en palabras.\n\n";
...
function buildPromptEN(cvText, liText, modo, role, sector, seniority, plan, situacion = '', includeProExtra = true) {
  ...
  instrBlock += "CRITICAL — DO NOT CONTRADICT THE DOCUMENT: before writing a weakness that says something is 'missing' or 'absent' (e.g. 'lacks quantified achievements', 'no metrics'), re-read the ENTIRE document looking for that information — not just the section or role you are describing at that moment. If the data DOES appear elsewhere in the resume (another role, another section), the weakness must be specific about WHERE it's missing ('this particular experience does not include metrics, unlike others in your resume'), never a general absence claim that contradicts what the document actually shows elsewhere. A weakness that contradicts the resume's real content is a serious error.\n\n";
  instrBlock += "ROLE VS. INDUSTRY — DO NOT CONFLATE THEM: identify the person's real ROLE or FUNCTION (what they do: researcher, analyst, coordinator, consultant, etc.) separately from the SECTOR or INDUSTRY where they applied it (real estate, mining, retail, tech). If the person held the same role or function across different sectors, THAT is the central fact of their profile — name it explicitly ('they are a social researcher, with experience applying that role in real estate, mining, retail, and tech'), do not dilute it into a generic 'adaptability across industries' observation that hides what their actual specialization is. This matters especially for unusual profiles or ones that don't follow a linear corporate career — do not force a generic reading when the document shows a clear, consistent specialization.\n\n";
  ...
  instrBlock += "HOW TO WRITE resumenEjecutivo (the main verdict): it is purely interpretive — do NOT include any number in this field, not even in parentheses. No atsScore, scorePotencial, impactDensityScore, or any other figure. Tell in words how the trajectory reads today, what works and what doesn't, and where it could move with the suggested adjustments — all in prose, without a single figure. Example of the tone to follow (adapt it to this document's real data, do not copy it literally): \"Today your trajectory is being read with [synthesis of how it reads: partial clarity / strongly / in a fragmented way, etc.] — your experience in [area] is [quality], but [what limits the impact, in words]. With the adjustments shown below, that reading could [improve significantly / change considerably]. What weighs most in that difference is [the concrete reason based on the document], which currently places your profile at a [high/medium/low] level of impact density.\"\n";
  instrBlock += "MANDATORY: before responding, review the resumenEjecutivo text and confirm it does not contain any digit (0-9) anywhere. If you find one, remove it and rewrite that part in words.\n\n";
